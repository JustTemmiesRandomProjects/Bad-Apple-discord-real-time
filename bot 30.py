...
for i in range(1, 232):
    path = "frames/frameframe/frame"+str(i*30+29)+".png"
...
async def on_message(ctx):
    ...
    if ctx.content.startswith('-apple'):
        
        i = 0
        
        bot.loop.create_task(ctx.send((frames[int(i)])))
        
        time.sleep(1)
        
        while i <= 695:
            ctx.send(frames[int(i)])
            time.sleep(1)
            i=i+1

    elif "bad apple" in ctx.content:
        await ctx.channel.send("did someone just mention bad apppplee??")
